...
BOOL _DSOUND::DSCreate(){
	...
	if(!LoadWave("waveData\\powan.wav",&lpDSSec[SOUND_JUMP]))
		return(FALSE);
	if(!LoadWave("waveData\\piko.wav",&lpDSSec[SOUND_HEAD]))
		return(FALSE);
	if(!LoadWave("waveData\\OH-YEA!.wav",&lpDSSec[SOUND_OPEN]))
		return(FALSE);
	if(!LoadWave("waveData\\koripon.wav",&lpDSSec[SOUND_HIT]))
		return(FALSE);
	if(!LoadWave("waveData\\hitBlock.wav",&lpDSSec[SOUND_BLKBREAK]))
		return(FALSE);
	if(!LoadWave("waveData\\pikopiko.wav",&lpDSSec[SOUND_PAUSE]))
		return(FALSE);
	if(!LoadWave("waveData\\gossan.wav",&lpDSSec[SOUND_CAGE]))
		return(FALSE);
	if(!LoadWave("waveData\\ufu1.wav",&lpDSSec[SOUND_DAMAGE]))
		return(FALSE);
	if(!LoadWave("waveData\\escape.wav",&lpDSSec[SOUND_ESCAPE]))
		return(FALSE);
	if(!LoadWave("waveData\\warning.wav",&lpDSSec[SOUND_WARNING]))
		return(FALSE);
	if(!LoadWave("waveData\\nandayo.wav",&lpDSSec[SOUND_GOVER]))
		return(FALSE);
	if(!LoadWave("waveData\\hartGet.wav",&lpDSSec[SOUND_ITEM]))
		return(FALSE);
	if(!LoadWave("waveData\\Alien Out.wav",&lpDSSec[SOUND_HITCAGE]))
		return(FALSE);
