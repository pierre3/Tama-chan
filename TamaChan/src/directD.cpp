...
BOOL _DDRAW::SetBitmap()
{
	...
	if(!DDLoadBmp(&lpDDOF[OFFSCRN_CHAR],"bitmap\\MyChar.bmp",
		10,236,BMP_X,BMP_MC_Y,BMP_MC_W,BMP_MC_H)){
		return(FALSE);
	}
	if(!DDLoadBmp(&lpDDOF[OFFSCRN_CHAR],"bitmap\\Block.bmp",
		10,236,BMP_X,BMP_BL_Y,BMP_BL_W,BMP_BL_H)){
		return(FALSE);
	}
	if(!DDLoadBmp(&lpDDOF[OFFSCRN_CHAR],"bitmap\\Enemy.bmp",
		10,236,BMP_X,BMP_EN_Y,BMP_EN_W,BMP_EN_H)){
		return(FALSE);
	}
	if(!DDLoadBmp(&lpDDOF[OFFSCRN_GOVER],"bitmap\\gover.bmp",
		10,236,0,0,OFF_GOVER_W,OFF_GOVER_H)){
		return(FALSE);
	}
	if(!DDLoadBmp(&lpDDOF[OFFSCRN_TITLE],"bitmap\\title.bmp",
